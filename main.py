...
if __name__ == '__main__':
    ...
    for p in ps:
        ...
        print(f'The difference between G and final_w for {p} dimensional representation is: ',difference)
        difference = frobeniu_norm_error(G.T @ G, final_w.T @ final_w)
        print(f'The difference between G^TG and W^TW for {p} dimensional representation is: ',difference)
        R = G.T @ final_w
        print('G.shape: ',G.shape, 'W.shape: ',final_w.shape)
        _,n = G.shape
        difference = frobeniu_norm_error(R @ R.T,np.identity(n))
        print(f'The difference between RR^T and I for {p} dimensional representation is: ',difference)
        difference = frobeniu_norm_error(R.T @ R,np.identity(n))
        print(f'The difference between R^TR and I for {p} dimensional representation is: ',difference)  
    ### END YOUR CODE 
